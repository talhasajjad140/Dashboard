import streamlit as st
import pandas as pd
import numpy as np
import os
import re

def _is_valid_data_file(file_path):
    """Skip files that are really HTML error pages or other bad downloads."""
    try:
        with open(file_path, 'rb') as f:
            head = f.read(8)
        if head.startswith(b'<!DOCTYP') or head.startswith(b'<html') or head.startswith(b'<HTML'):
            return False
        return True
    except:
        return False


def convert_damage(value):
    """Convert damage strings like '10K' or '2.5M' to numbers"""
    if pd.isna(value) or value == '' or value == 0:
        return 0
    
    if isinstance(value, (int, float)):
        return float(value)
    
    try:
        val_str = str(value).strip().upper()
        multipliers = {'K': 1000, 'M': 1000000, 'B': 1000000000}
        
        # check if last char is a multiplier
        if val_str[-1] in multipliers:
            num = float(val_str[:-1])
            return num * multipliers[val_str[-1]]
        else:
            return float(val_str)
    except:
        return 0


@st.cache_data
def load_data(filepath):
    """Load and clean storm events data from one file or a folder"""
    # use os.path so local + Streamlit Cloud both work
    full_path = os.path.join(os.getcwd(), filepath)

    # if a combined CSV exists, use that first
    if os.path.isdir(full_path):
        combined_files = [
            os.path.join(full_path, name)
            for name in os.listdir(full_path)
            if name.lower().startswith('stormevents_combined_') and name.lower().endswith('.csv')
        ]
        if combined_files:
            latest_file = sorted(combined_files, key=os.path.getmtime)[-1]
            df = pd.read_csv(latest_file, low_memory=False)
            source_name = os.path.basename(latest_file)
            df['_source_file'] = source_name
            return _clean_data(df)

    files_to_read = []
    if os.path.isdir(full_path):
        for name in os.listdir(full_path):
            lower_name = name.lower()
            if ('details' in lower_name) and (lower_name.endswith('.csv') or lower_name.endswith('.csv.gz')):
                candidate = os.path.join(full_path, name)
                if _is_valid_data_file(candidate):
                    files_to_read.append(candidate)
    else:
        if _is_valid_data_file(full_path):
            files_to_read.append(full_path)

    if not files_to_read:
        raise FileNotFoundError(f"No CSV files found in: {filepath}")

    parts = []
    for file_path in sorted(files_to_read):
        part = pd.read_csv(file_path, low_memory=False, compression='infer')
        # keep track of source filename for inference if needed
        part['_source_file'] = os.path.basename(file_path)
        parts.append(part)

    df = pd.concat(parts, ignore_index=True)
    return _clean_data(df)


def _clean_data(df):
    # clean column names - lowercase and strip spaces
    df.columns = df.columns.str.strip().str.lower()

    # NOAA files can have begin_date_time or begin_date
    if 'begin_date_time' in df.columns:
        df['begin_date_time'] = pd.to_datetime(df['begin_date_time'], errors='coerce')
        df['begin_date'] = df['begin_date_time']
    elif 'begin_date' in df.columns:
        df['begin_date'] = pd.to_datetime(df['begin_date'], errors='coerce')

    if 'end_date_time' in df.columns:
        df['end_date_time'] = pd.to_datetime(df['end_date_time'], errors='coerce')
        df['end_date'] = df['end_date_time']
    elif 'end_date' in df.columns:
        df['end_date'] = pd.to_datetime(df['end_date'], errors='coerce')

    damage_cols = ['damage_property', 'damage_crops']
    for col in damage_cols:
        if col in df.columns:
            df[col] = df[col].apply(convert_damage)

    if 'begin_date' in df.columns:
        df['year'] = df['begin_date'].dt.year
    else:
        df['year'] = np.nan

    if '_source_file' in df.columns:
        def _year_from_filename(name):
            if not isinstance(name, str):
                return None
            m = re.search(r'd(20\d{2}|19\d{2})', name)
            if m:
                try:
                    return int(m.group(1))
                except:
                    return None
            return None

        inferred = df['_source_file'].apply(_year_from_filename)
        df['year'] = df['year'].fillna(inferred)

    if 'fatality_location' in df.columns and 'state' not in df.columns:
        df['state'] = df['fatality_location']

    if 'fatality_type' in df.columns and 'event_type' not in df.columns:
        df['event_type'] = np.where(df['fatality_type'].isna(), 'Unknown', df['fatality_type'])

    required_numeric = ['damage_property', 'damage_crops', 'deaths_direct', 'injuries_direct', 'magnitude']
    for col in required_numeric:
        if col not in df.columns:
            df[col] = 0

    if 'event_type' in df.columns:
        df['event_type'] = df['event_type'].astype(str).str.strip()
    if 'state' in df.columns:
        df['state'] = df['state'].astype(str).str.strip()

    damage_cols_exist = [col for col in damage_cols if col in df.columns]
    for col in damage_cols_exist:
        df[col] = np.where(df[col].isna(), 0, df[col])

    return df


def get_unique_values(df, column):
    """Get sorted unique values from a column for dropdowns"""
    if column not in df.columns:
        return []

    vals = df[column].dropna().astype(str).str.strip()
    vals = vals[vals != '']
    return sorted(vals.unique())


def apply_filters(df, year_range, states, event_types, 
                  search_term, damage_range):
    """Apply all sidebar filters to the dataframe"""
    filtered = df.copy()
    
    # year filter
    if year_range and 'year' in filtered.columns:
        filtered = filtered[(filtered['year'] >= year_range[0]) & 
                           (filtered['year'] <= year_range[1])]
    
    # states filter
    if states and 'state' in filtered.columns:
        filtered = filtered[filtered['state'].isin(states)]
    
    # event types filter
    if event_types and 'event_type' in filtered.columns:
        filtered = filtered[filtered['event_type'].isin(event_types)]
    
    # property damage range filter
    if damage_range and 'damage_property' in filtered.columns:
        filtered = filtered[(filtered['damage_property'] >= damage_range[0]) & 
                           (filtered['damage_property'] <= damage_range[1])]
    
    # search term - searches event_type and state columns
    if search_term and search_term.strip() and 'event_type' in filtered.columns and 'state' in filtered.columns:
        search_lower = search_term.lower().strip()
        filtered = filtered[
            (filtered['event_type'].str.lower().str.contains(search_lower, na=False)) |
            (filtered['state'].str.lower().str.contains(search_lower, na=False))
        ]
    
    return filtered
